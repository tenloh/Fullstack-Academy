...
function problemA () {
  ...
  Promise.all([stanza1,stanza2]).then(contents => {
    blue(contents[0]); 
    blue(contents[1]);
    console.log('done');
  });
...
function problemB () {
  ...
  const filenames = [1, 2, 3, 4, 5, 6, 7, 8].map(n => {return `poem-two/stanza-0${n}.txt`;});
  const readFiles = filenames.map(f => promisifiedReadFile(f));

  Promise.all(readFiles).then(function(contents){
    contents.forEach(stanza => blue(stanza));
    console.log('done');
...
/*  var filenames = [1, 2, 3, 4, 5, 6, 7, 8].map(function (n) {
  ...
  Promise.each(readFiles, function(stanza){
    blue(stanza);
  }, function(err){magenta(err)}).finally(function(done){console.log('done')});
}
...
function problemE () {
  ...
  function promisifiedWriteFile (filename, str) {
    ...
    writeFile(filename, str).then(function(value){
    ...
    Promise.promisify(fs.writeFile)(filename, str)
    .then((value) => { console.log(`done writing ${value}`) }, (err) => {console.log(err)} );
    
  }

  promisifiedWriteFile('test.txt', 'asdfasdfqwrfwqe');
}
